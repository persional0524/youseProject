-- 步骤类型: N/A, 步骤名称: 存量银行承兑汇票信息__DW查询
-- 标签: name, 属性: {}, 内容: 存量银行承兑汇票信息__DW查询
...
select '${reportId}' REPORT_ID,
    ...
    nvl(ywht.margin_rt,0) bzjbl, 	--保证金比例
    ct.sure_mode as dbfs,               		--担保方式
    pj.v_dept_id v_dept_id,			--部门编号
	'System' creater              	--创建人
	--aux_expenses	number(20,2)	y		费用金额
from dh_bms_pjxx pj
join dh_fb_pjcd tx on tx.billno = pj.billno
left join dh_fb_ywht ywht 
on ywht.contract_no = tx.contract_no and 
   ywht.ht_s_dt <= date '${endDate}' and 
   ywht.ht_e_dt >  date '${endDate}' 
left join (
	select billno,
		contract_no,
		sum(aux_expenses) aux_expenses
	from dh_bms_detail
	where tx_type = 2 
	  and tx_date <= date '${endDate}'
	group by billno,contract_no
) jymx on pj.billno = jymx.billno
-- 出票人
...
where acceptor.cltno = org.org_inter_customer_code  --承兑人为财务公司的票据
  and pj.accptmode = 0								--承兑方式 0-银承
  and pj.status = 1									--票据状态 1-正常
  and pj.ht_s_dt <= date '${endDate}'
  and pj.ht_e_dt >  date '${endDate}' 
  and ywht.start_date <= date '${endDate}'
  and ywht.biz_type= 118
