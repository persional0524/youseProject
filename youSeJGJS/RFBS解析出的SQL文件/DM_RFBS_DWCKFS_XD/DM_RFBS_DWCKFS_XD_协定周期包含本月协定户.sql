-- 步骤类型: N/A, 步骤名称: 协定周期包含本月协定户
-- 标签: name, 属性: {}, 内容: 协定周期包含本月协定户
...
xdjy_base as (
  ...
  on cd.deposit_id = xdxy.deposit_id and   
     ...
     xdxy.xd_sdate < to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd') and    --协议开始日期小于月初 
     xdxy.xd_edate > date '${endDate}'   --协议结束日期大于月末
  ...
  left join (
    ...
    on a.deposit_id = b.deposit_id and 
       ...
       a.xd_sdate < to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd') and   --协议开始日期小于月初 
       a.xd_edate > date '${endDate}' and   --协议结束日期大于月末
  ...
  left join (
    ...
    on t.deposit_id = d.deposit_id and 
       t.ht_s_dt <= aa.book_date and 
       t.ht_e_dt >  aa.book_date
