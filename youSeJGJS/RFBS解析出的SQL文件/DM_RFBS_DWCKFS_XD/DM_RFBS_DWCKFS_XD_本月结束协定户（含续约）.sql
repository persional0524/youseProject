-- 步骤类型: N/A, 步骤名称: 本月结束协定户（含续约）
-- 标签: name, 属性: {}, 内容: 本月结束协定户（含续约）
...
xdjy_base as (
  ...
  on cd.deposit_id = xdxy.deposit_id and 
     xdxy.xd_edate >= to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd') and        --协议结束日期大于月初 
     xdxy.xd_edate <= date '${endDate}' and                                             --协议结束日期小于月末 
     xdxy.ht_s_dt <= jymx.book_date and 
     xdxy.ht_e_dt >  jymx.book_date
  left join  (
    ...
    on a.deposit_id = b.deposit_id and  
       to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd')-1 >= b.tx_date and 
  ...
  left join (
    ...
    inner join dh_sm_ckcd cd1 
    on aa.deposit_id=cd1.deposit_id and 
       cd1.biz_type_id='A0101' and          --协定结束未续约的 业务品种变成a01
       cd1.ht_s_dt <= aa.book_date and 
       cd1.ht_e_dt >  aa.book_date
    inner join dh_sm_agreement xdxy
    on cd1.deposit_id = xdxy.deposit_id and 
       xdxy.xd_edate >= to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd') and    --协议结束日期大于月初 
       xdxy.xd_edate <= date '${endDate}'   --协议结束日期小于月末 
    left join dh_sm_ckjymx bb
    on aa.deposit_id = bb.deposit_id and 
       bb.book_date <= date '${endDate}' and 
       bb.book_date >= to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd') and 
       bb.trade_time <= aa.trade_time
    where aa.book_date <= date '${endDate}'
      and aa.book_date>=to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd')
  ...
  where jymx.book_date >= to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd')
    and jymx.book_date <= xdxy.xd_edate
  order by jymx.deposit_id,jymx.trade_time  
),
xdjy_sol as (
  select jylsh, --交易流水号
...
SELECT    
  ...
  xdjy_base.ckbz,                                           --币种
  xdjy_sol.xdhjyje ckfsje,                                  --存款发生金额
  case when xdjy_base.ckbz = 'CNY' then xdjy_sol.xdhjyje
    when xdjy_base.ckbz != 'CNY' then xdjy_sol.xdhjyje * (case when (select s.value_ from dm_report_param_set s where s.bspzbh='RFBS' and s.key_='AVG_EXCHANGE_RATE') = '1' 
