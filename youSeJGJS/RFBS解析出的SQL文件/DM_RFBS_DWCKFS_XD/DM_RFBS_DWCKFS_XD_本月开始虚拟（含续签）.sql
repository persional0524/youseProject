-- 步骤类型: N/A, 步骤名称: 本月开始虚拟（含续签）
-- 标签: name, 属性: {}, 内容: 本月开始虚拟（含续签）
...
from (
  select
    ...
    case when Z.BZ = 'CNY' then ( case when b.balance>=xdxy.xded then xdxy.xded else b.balance end ) 
      when Z.BZ != 'CNY' then ( case when b.balance>=xdxy.xded then xdxy.xded else  b.balance end ) * 
                              ( case when (select s.value_ from dm_report_param_set s where s.bspzbh='RFBS' and s.key_='AVG_EXCHANGE_RATE') = '1' 
    ...
    xdxy.xdll llsp,                --利率水平
    to_char(xdxy.xd_sdate,'yyyy-mm-dd') jyrq,          --交易日期
    b.deposit_id||'BEGIN01' jylsh,                     --交易流水号
    '1' jyfx,                      --交易方向  
    '99' jyqd,                     --交易渠道
    '0' xjzzbs,                    --现金转账标识
...
where xdxy.xd_sdate >= to_date((substr('${endDate}',1,8)||'01'),'yyyy-mm-dd')
  and xdxy.xd_sdate <= date '${endDate}'  
...
select
  ...
  to_char(cd.acct_end_date,'yyyy-mm-dd') ckxysjzzrq,                        --存款协议实际终止日期
  z.bz ckbz,                        --币种
  b.balance-xdxy.xded ckfsje,       --存款发生金额
  case when z.bz = 'CNY' then b.balance-xdxy.xded 
    when z.bz != 'CNY' then (b.balance-xdxy.xded) *
                            (case when (select s.value_ from dm_report_param_set s where s.bspzbh='RFBS' and s.key_='AVG_EXCHANGE_RATE') = '1' 
                                  then nvl((select avg_rate from avg_exchange_rate where cur_orig = z.bz and cur_dest = 'CNY' and sjrq = '${endDate}'),
                                           (select avg(hl) from dh_gl_hlb where ht_e_dt >= add_months(last_day(date'${endDate}'),-1)+1 and ht_s_dt <= date'${endDate}' and cur_orig = z.bz and cur_dest = 'CNY'))
                                  when (select s.value_ from dm_report_param_set s where s.bspzbh='RFBS' and s.key_='AVG_EXCHANGE_RATE') = '0' 
  ...
  xdxy.xdll llsp,                   --利率水平
  to_char(xdxy.xd_sdate,'yyyy-mm-dd') jyrq,     --交易日期
  b.deposit_id||'BEGIN02' jylsh,                --交易流水号
  '1' jyfx,                         --交易方向  虚拟活期转出
  '99' jyqd,                        --交易渠道
  '0' xjzzbs,                       --现金转账标识
  c.chinese_name jydsmc,            --交易对手名称
  ...
  c.op_bank_code jydszhkhhh,        --交易对手账户开户行号
  czj.cust_type jydszjlx,           --客户证件类型
  czj.cust_type jydszjdm,           --客户证件代码 
  '签订协定协议后从活期转入协定户' ckjyyt          --存款交易用途
from dh_sm_agreement xdxy
left join (
  select max(tx_date) max_date,a.deposit_id 
  from dh_sm_agreement a
...
select    
  ...
  to_char(xdxy.xd_sdate,'yyyy-mm-dd') jyrq,         --交易日期
  b.deposit_id||'BEGIN03' jylsh,    --交易流水号
  '0' jyfx,                         --交易方向  虚拟活期转出
  '99' jyqd,                        --交易渠道
  '0' xjzzbs,                       --现金转账标识
  c.chinese_name jydsmc,            --交易对手名称
