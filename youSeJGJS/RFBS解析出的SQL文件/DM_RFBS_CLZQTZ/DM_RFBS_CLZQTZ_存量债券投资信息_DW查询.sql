-- 步骤类型: N/A, 步骤名称: 存量债券投资信息_DW查询
-- 标签: name, 属性: {}, 内容: 存量债券投资信息_DW查询
...
select p.paramvalue,p.paramname
  ...
  select SEQ_DM_RFBS_CLZQTZ.NEXTVAL ID,
         '${reportId}' REPORT_ID,
         '${BATCH}' BATCH,
         u.*
         from
        ...
        when ba.FINANCE_NO='2000004.IB' then '400'
          ...
          ba.CUR_CODE BZ,--币种
          sum(ba.BOOK_VALUE) ZQYE,--债券余额
          sum(ba.BOOK_VALUE)*PKG_RMM_FUNCTIONS.CurrencyRate(ba.CUR_CODE,date'${endDate}') ZQYEZRMB,--债券余额折人民币
		...
		nvl(ba.ECONOMIC_SECTOR,clt.Economic_Sector) END FHRGMJJBM --发行人国民经济部门
          from DH_INVEST_BASEINFO ba
          left join DH_INVEST_BOND bo on ba.TRADE_ID = bo.TRADE_ID
  ...
  ON M2.APPNO = 'RIMAS' AND 
     ...
     M2.STANDARD_CODE = clt.QYKGLX
          ...
          ba.product_type,
          ba.CUR_CODE,
          bo.DEBE_CREDIT_RATING,
          ba.ACT_DATE,
          ba.ACT_DATE,
          ba.ACCT_END_DATE,ba.END_DATE,
          ba.RATE ,
          ba.CREDIT_CODE ,
          ba.ISSUER_COUNTRY,
          ba.product_type ,
          ...
          ba.ISSUER_INDUSTRY,    
          ba.ISSUER_SCALE,
          ba.ECONOMIC_INGREDIENT,
          ba.ECONOMIC_SECTOR,
          clt.ENTERPRISE_SCALE,clt.INDUSTRY ,clt.ADMIN_DIVISION,M2.PARAMVALUE,clt.Economic_Sector )u
