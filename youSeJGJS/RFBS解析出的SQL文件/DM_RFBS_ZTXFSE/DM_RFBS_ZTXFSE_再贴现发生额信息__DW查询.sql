-- 步骤类型: N/A, 步骤名称: 再贴现发生额信息__DW查询
-- 标签: name, 属性: {}, 内容: 再贴现发生额信息__DW查询
...
where a.CPRZJDM is not null
 ...
 SELECT '${reportId}' as REPORT_ID,
        ...
        TO_CHAR(DE.TX_DATE,'YYYY-MM-DD') AS JYRQ ,  --交易日期
  ...
  PJ.ACCEPTORCREDITCODE AS  CDRZJDM,--承兑人证件代码
       ...
       /* NVL(PJ.AMOUNT,0)*PKG_RMM_FUNCTIONS.CURRENCYRATE_NEW(PJ.CURE_CODE, RE.CURRENCY_NO, DATE '${endDate}')-NVL(DE.ACCRUED_INTEREST,0)-NVL(DE.AUX_EXPENSES,0) AS ZTXJE, --再贴现金额
       NVL(CASE WHEN  RE.CURRENCY_NO =  'CNY' THEN NVL(PJ.AMOUNT,0)*PKG_RMM_FUNCTIONS.CURRENCYRATE_NEW(PJ.CURE_CODE, RE.CURRENCY_NO, DATE '${endDate}')-NVL(DE.ACCRUED_INTEREST,0)-NVL(DE.AUX_EXPENSES,0)
             WHEN  RE.CURRENCY_NO <> 'CNY' 
             THEN (NVL(PJ.AMOUNT,0)*PKG_RMM_FUNCTIONS.CURRENCYRATE_NEW(PJ.CURE_CODE, RE.CURRENCY_NO, DATE '${endDate}')-NVL(DE.ACCRUED_INTEREST,0)-NVL(DE.AUX_EXPENSES,0)) * PKG_RMM_FUNCTIONS.CURRENCYRATE_NEW( RE.CURRENCY_NO,'CNY', DATE '${endDate}') 
    ...
    and jl.actual_date<=DE.TX_DATE and jl.actual_date>=re.start_date  and jl.bustype =3) from dual)   AS ZTXJE, --再贴现金额
     (select re.ACCT_AMOUNT+(select nvl(sum(jl.interest),0) from dh_fb_lxtxjl jl where jl.BUS_ID = PJCD.NOTE_NO
    and jl.actual_date<=DE.TX_DATE and jl.actual_date>=re.start_date  and jl.bustype =3) from dual)   AS ZTXJEZRMB     ,               --再贴现金额折人民币
       CASE WHEN DE.TX_TYPE ='10' THEN '1' --再贴现->发生
            WHEN DE.TX_TYPE ='6'  THEN '0' --赎回->结清
       END  JYFX   ,                        --交易方向 
       trim(DE.FLOW_ID) as JYLSH                  --交易流水号
FROM DH_BMS_PJXX PJ
INNER JOIN DH_FB_TXPJ PJCD ON PJCD.NOTE_NO = PJ.BILLNO  
--INNER JOIN DH_REDISCOUNT_INFO RE ON PJCD.CONTRACT_NO=RE.CONTRACT_NO AND RE.STATE= 1 
INNER JOIN DH_REDISCOUNT_INFO RE ON PJCD.bid=RE.bid --AND RE.STATE= 1 
INNER JOIN DH_BMS_DETAIL DE ON DE.BILLNO = PJ.BILLNO  AND DE.TX_TYPE IN ('6','10')
-- 出票人
LEFT JOIN CLT CPR
...
WHERE   re.start_date <= DATE '${endDate}'   ---PJ.STATUS = 1                  --票据状态 1-正常
 ...
 and re.maturity >= date'${startDate}'
