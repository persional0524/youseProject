...
with head as (
 select org.ORG_CLTNO NBJGH,
  ...
  JYDS as (
  select distinct a.CERT_TYPE_CODE, a.CERT_NUM, a.OP_NAME, a.OP_TYPE  from CUST_OP_INFO@DCAB_DM  a right outer join (
         select OP_NAME, max(CREATE_TIME) CREATE_TIME  from CUST_OP_INFO@DCAB_DM group by OP_NAME
         ) b on a.CREATE_TIME=b.CREATE_TIME and a.OP_NAME=b.OP_NAME
  ),
  YHHH as -- 银行行号
...
select  '${reportId}' REPORT_ID,
 ...
 from (
  ...
  when 1 then decode(c.DISTRICT,'542225','540524','542221','540502','652201','650500',c.DISTRICT) --琼结县转码值 
    ...
    case when Z.XHRQ <='${endDate}' then Z.XHRQ else null end  CKXYSJZZRQ,--存款协议实际终止日期
    Z.BZ  CKBZ,--币种
...
and z.mxkmbh not like '2012%'
    ...
    D.CERTNUMBER    CKXYDM,--存款协议代码
    case 
       when D.BIZ_TYPE_ID in ('D061','0001','0005') then 'D061' --银行承兑汇票保证金存款
       when D.BIZ_TYPE_ID in ('D062','0002','0006') then 'D062' --信用证保证金存款
       when D.BIZ_TYPE_ID in ('D063','0004','0008') then 'D063' --保函保证金存款
       ...
       when D.BIZ_TYPE_ID in ('D069','0003','0007','9999') then 'D069' --其他保证金存款
    end  CKCPLB,--存款产品类别
    to_char(D.BEGIN_DATE,'YYYY-MM-DD')  CKXYQSRQ,--存款协议起始日期
    --to_char(D.END_DATE,'YYYY-MM-DD')  CKXYDQRQ,--存款协议到期日期
