
  CREATE TABLE "YSKY_SMS"."CLT_CLT_INFO" 
   (	"ID" NUMERIC(6,0) NOT NULL ENABLE, 
	"TYPE" NUMERIC(*,0), 
	"CLTNO" VARCHAR2(30), 
	"TREENO" VARCHAR2(24), 
	"CHINESENAME" VARCHAR2(100), 
	"ABBREVIATION" VARCHAR2(100), 
	"NAME" VARCHAR2(100), 
	"OPBANK" VARCHAR2(80), 
	"ACNTNO" VARCHAR2(32), 
	"INTERNETSITE" VARCHAR2(30), 
	"PHONE" VARCHAR2(60), 
	"BUSSADDR" VARCHAR2(255), 
	"POSTCODE" VARCHAR2(6), 
	"FAX" VARCHAR2(30), 
	"DEPINCHARGE" VARCHAR2(60), 
	"DAYOFESTB" DATE, 
	"INDUSTRY" NUMERIC(*,0), 
	"RANKING" NUMERIC(*,0), 
	"ORGCODE" VARCHAR2(30), 
	"FINANTYPE" NUMERIC(*,0), 
	"LISENSENO" VARCHAR2(50), 
	"TAXREGNO" VARCHAR2(50), 
	"LOANNO" VARCHAR2(50), 
	"OPRATETERM" VARCHAR2(20), 
	"FERC" VARCHAR2(50), 
	"BUSSSCOPE" VARCHAR2(2000), 
	"REGCAPITAL" NUMERIC(16,2), 
	"CUR" VARCHAR2(8), 
	"REGADDR" VARCHAR2(100), 
	"LEGALPERZIP" VARCHAR2(10), 
	"LEGALPER" VARCHAR2(60), 
	"LEGALPERID" VARCHAR2(18), 
	"LEGALPERPHONE" VARCHAR2(30), 
	"CREDITLEVEL" NUMERIC(*,0), 
	"EVALAUTH" VARCHAR2(60), 
	"EVALTIME" DATE, 
	"CREDITRATING" NUMERIC(*,0), 
	"EXTCLTNO" VARCHAR2(30), 
	"ISLISTED" NUMERIC(*,0), 
	"AREA" NUMERIC(*,0), 
	"COUNTRYCODE" CHAR(3), 
	"INGROUP" VARCHAR2(100), 
	"NATURE" NUMERIC(*,0), 
	"CORPTYPE" NUMERIC(*,0), 
	"CORSUBTYPE" NUMERIC(*,0), 
	"OTHER" VARCHAR2(1024), 
	"BANKNO" VARCHAR2(30), 
	"CREATOR" VARCHAR2(24), 
	"CREATETIME" DATE, 
	"STAUTS" NUMERIC(*,0), 
	"CAUSE" VARCHAR2(200), 
	"ENTERPRISESCALE" NUMERIC(*,0), 
	"LICENSECODE" VARCHAR2(30), 
	"FINANCIALCODE" VARCHAR2(20), 
	"CLTSTATE" NUMERIC(*,0) DEFAULT 0, 
	"EMAIL" VARCHAR2(50), 
	"FINANCIALCONTACT" VARCHAR2(50), 
	"ENDDATE" DATE, 
	"STOCKCODE" VARCHAR2(50), 
	"SITEAREA" NUMERIC(15,4), 
	"SITETITLE" VARCHAR2(50), 
	"EMPLOYEESNUM" NUMERIC(*,0), 
	"TAXNUM" VARCHAR2(50), 
	"LANDTAXNUM" VARCHAR2(50), 
	"PRODUCTSMAJOR" VARCHAR2(50), 
	"LISTEDPLACE" VARCHAR2(50), 
	"PROVINCE" VARCHAR2(50), 
	"CITY" VARCHAR2(50), 
	"DISTRICT" VARCHAR2(50), 
	"BORRNATURE" NUMERIC(*,0), 
	"MECCODE" VARCHAR2(50), 
	"PROJECTNAME" VARCHAR2(200), 
	"COMPANTYATTRIBUTE" NUMERIC(*,0), 
	"REGIONTEXT" VARCHAR2(200), 
	"LEFTFIG_N3" NUMERIC(38,0), 
	"RIGHTFIG_N3" NUMERIC(38,0), 
	"DEPT_TYPE" VARCHAR2(50), 
	"ORG_PARENT_ID" VARCHAR2(32), 
	"ROLEGROUPID" NUMERIC(*,0), 
	"FINA_PRINCIPAL" VARCHAR2(30), 
	"PRIMARY_ACC_NO" VARCHAR2(32), 
	"CONTACT_PERSON" VARCHAR2(80), 
	"REMARK_" VARCHAR2(500), 
	"SYNC_FLAG" NUMERIC(*,0), 
	"SYNC_VERSION" NUMERIC(*,0), 
	"AUTH_BY_USER_FLAG" NUMERIC(*,0), 
	"AGENCY_NO" VARCHAR2(32), 
	"STATUS" CHAR(1), 
	"ORG_ORDER" NUMERIC, 
	"ORG_LEVEL" NUMERIC, 
	"MOBILE_PHONE" VARCHAR2(32), 
	"ORGCLASSID" NUMERIC(18,0) DEFAULT 0, 
	"BRNO" VARCHAR2(4), 
	"BASE_ACCOUNT_STATE" VARCHAR2(20), 
	"INVOICERECADDR" VARCHAR2(50), 
	"INVOICERECPHONE" VARCHAR2(20), 
	"INVOICERECNAME" VARCHAR2(32), 
	"FORTDRTFG" VARCHAR2(20), 
	"BCOMMADD" VARCHAR2(80), 
	"ORGTYPEDETAIL" VARCHAR2(20), 
	"ORGTYPE" VARCHAR2(20), 
	"APPRONUM" VARCHAR2(20), 
	"REGNUMCODE" VARCHAR2(20), 
	"REGCODE" VARCHAR2(20), 
	"INSTICREDITCODE" VARCHAR2(20), 
	"MANAGEBANKCODE" VARCHAR2(20), 
	"CLTTYPE" VARCHAR2(20), 
	"PARENT_ID" NUMERIC(6,0), 
	"ORGCODE_ENDDATE" DATE, 
	"REREDT" DATE, 
	"CURDFLAG" NUMERIC(1,0) DEFAULT 0, 
	"DETAILEDTYPE" VARCHAR2(1000), 
	"THREECARDBYONE" VARCHAR2(100), 
	"THREECARDBYONE_DATE" DATE, 
	"APPRO_DATE" DATE, 
	"INSTICREDITCODE_DATE" DATE, 
	"SEOPENINGBANK" VARCHAR2(1000), 
	"SENUMERIC" VARCHAR2(1000), 
	"LEGALPERIDTYPE" NUMERIC(*,0), 
	"LEGALPERID_DATE" DATE, 
	"REALHOLDINGNAME" VARCHAR2(1000), 
	"REALHOLDINGTYPE" NUMERIC(*,0), 
	"REALHOLDINGCARDNUMERIC" VARCHAR2(50), 
	"REALHOLDINGCARD_DATE" DATE, 
	"SHAREHOLDER_ONE_NAME" VARCHAR2(50), 
	"SHAREHOLDER_ONE_CARDTYPE" NUMERIC(*,0), 
	"SHAREHOLDER_ONE_CARDNUMERIC" VARCHAR2(50), 
	"SHAREHOLDER_ONE_CARDDATE" DATE, 
	"SHAREHOLDER_TWO_NAME" VARCHAR2(50), 
	"SHAREHOLDER_TWO_CARDTYPE" NUMERIC(*,0), 
	"SHAREHOLDER_TWO_CARDNUMERIC" VARCHAR2(50), 
	"SHAREHOLDER_TWO_CARDDATE" DATE, 
	"AGENTNAME" VARCHAR2(50), 
	"AGENT_CARDTYPE" NUMERIC(*,0), 
	"AGENT_CARDNUMERIC" VARCHAR2(50), 
	"AGENT_CARDDATE" DATE, 
	"CBRCBUSINESSID" VARCHAR2(1000), 
	"U_SHIELD_DATE" DATE, 
	"ACCOUNT_NATURE" NUMERIC(*,0), 
	"ACCOUNT_STATUS" NUMERIC(*,0), 
	"CANCELLATION_DATE" DATE, 
	"LASTCHANGE_DATE" DATE, 
	"SHAREMETHOD" VARCHAR2(30), 
	"ISENCRYPTY" VARCHAR2(2), 
	"REGISTERED" VARCHAR2(10), 
	"ECONOMYDEPT" VARCHAR2(50), 
	"HOLDINGTYPE" VARCHAR2(50), 
	"RURALCITYSIGNS" VARCHAR2(50) DEFAULT 'N', 
	"INSTITUTIONALLEVEL" VARCHAR2(50), 
	"PARENTINSTITUTIONSNO" VARCHAR2(50), 
	"FINANCIALINSTITUTIONTYPE" VARCHAR2(50), 
	 CONSTRAINT "PK_CLT_CLT_INFO" PRIMARY KEY ("ID")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1
  BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "T003"  ENABLE, 
	 CONSTRAINT "UK_CLT_CLT_INFO_CLTNO" UNIQUE ("CLTNO")
  USING INDEX PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS 
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1
  BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "T003"  ENABLE, 
	 CONSTRAINT "FK_CLT_REF_CONF2" FOREIGN KEY ("CREDITLEVEL")
	  REFERENCES "YSKY_SMS"."CLT_CONF_INFO" ("ID") ENABLE, 
	 CONSTRAINT "FK_CLT_REF_CONF1" FOREIGN KEY ("INDUSTRY")
	  REFERENCES "YSKY_SMS"."CLT_CONF_INFO" ("ID") ENABLE, 
	 CONSTRAINT "FK_CLT_REF_CONF3" FOREIGN KEY ("CREDITRATING")
	  REFERENCES "YSKY_SMS"."CLT_CONF_INFO" ("ID") ENABLE
   ) SEGMENT CREATION IMMEDIATE 
  PCTFREE 10 PCTUSED 40 INITRANS 1 MAXTRANS 255 
 NOCOMPRESS LOGGING
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
  PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1
  BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)
  TABLESPACE "USERS" ;
